		...
		if (required.isPresent()) {
			...
			if (runningVersion.isGreaterThanOrEqualTo(requiredVersion)) {
				return ConditionEvaluationResult
					.enabled("Test is enabled, @VaultVersion(%s) is met with Vault running version %s".formatted(
                        requiredVersion, runningVersion));
			}

			return ConditionEvaluationResult
				.disabled("Test is disabled, @VaultVersion(%s) is not met with Vault running version %s".formatted(
                    requiredVersion, runningVersion));
		}
		...
		if (disabled.isPresent()) {
			...
			if (runningVersion.isGreaterThanOrEqualTo(disabledVersion)) {
				return ConditionEvaluationResult.disabled(
                        "Test is disabled, @DisabledOnVaultVersion(%s) is met with Vault running version %s".formatted(
                                disabledVersion, runningVersion));
			}

			return ConditionEvaluationResult.enabled(
                    "Test is enabled, @DisabledOnVaultVersion(%s) is not met with Vault running version %s".formatted(
                            disabledVersion, runningVersion).formatted());
		}
